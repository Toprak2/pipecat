...
class MetricsLogObserver(BaseObserver):
    ...
    Examples:
        ...
        Exclude specific metrics types::

            from pipecat.metrics.metrics import ProcessingMetricsData
            observers = [
                MetricsLogObserver(exclude_metrics={ProcessingMetricsData})
            ]
    """
    ...
    def __init__(
        ...
        exclude_metrics: Optional[Set[Type[MetricsData]]] = None,
        **kwargs,
    ):
        ...
        Args:
            include_metrics: Set of metrics types to include. If specified, only these
                metrics types will be logged. If None, all metrics are logged (unless
                excluded via exclude_metrics).
            exclude_metrics: Set of metrics types to exclude from logging. Ignored if
                include_metrics is specified.
            **kwargs: Additional arguments passed to parent class.
        """
        super().__init__(**kwargs)
        self._include_metrics = include_metrics
        self._exclude_metrics = exclude_metrics or set()
    ...
    def _should_log_metrics(self, metrics_data: MetricsData) -> bool:
        ...
        """
        metrics_type = type(metrics_data)

        # If include_metrics is specified, only log those types
        if self._include_metrics is not None:
            return metrics_type in self._include_metrics

        # Otherwise, log everything except excluded types
        return metrics_type not in self._exclude_metrics
